...
class DataPipeline:
    ...
    def _load_employee_dimension(self, clear_existing=False):
        ...
        # Column detection for Excel file structure:
        # Column A (ID): numeric ID for self-join
        # Column B (NAME): employee name
        # Column D (MANAGER ID): numeric ID of manager (references Column A)
        # Column E (wire code): wire code identifier
        
        id_cols = ['ID', 'id', 'Id']
        rm_name_cols = ['NAME', 'RM_Name', 'RM Name', 'rm_name']
        manager_id_cols = ['MANAGER ID', 'Manager ID', 'manager_id', 'ManagerID']
        wire_code_cols = ['wire code', 'WireCode', 'Wire Code', 'wire_code']
                ...
                try:
                    ...
                    for sheet_name in xls.sheet_names:
                        ...
                        for idx, row in df.iterrows():
                            try:
                                # Get numeric ID (Column A) - PRIMARY KEY
                                emp_id = None
                                for col in id_cols:
                                    val = row.get(col)
                                    if not pd.isna(val):
                                ...
                                if not emp_id or not rm_name:
                                    logger.debug(f"    Row {idx}: Skipped (missing ID or NAME)")
                                    continue
                                
                                # Get MANAGER ID (Column D) - FOREIGN KEY
                                manager_id = None
                                for col in manager_id_cols:
                                    val = row.get(col)
                                    if not pd.isna(val) and str(val).strip().lower() != 'null':
                                        try:
                                            manager_id = int(val)
                                            break
                                        except (ValueError, TypeError):
                                ...
                                # Create/Update Employee with numeric ID
                                emp = Employee.objects.update_or_create(
                                    id=emp_id,
                                    defaults={
                                        'wire_code': wire_code or f'EMP_{emp_id}',
                                        'rm_name': rm_name,
                                        'manager_id': manager_id,  # Numeric FK to another Employee.id
                                ...
                                count += 1
                                logger.info(f"    Row {idx}: ID={emp_id}, Name={rm_name}, Manager={manager_id}")
                            except Exception as e:
    ...
    def _build_employee_hierarchy(self):
        """
        Validate and log employee hierarchy (self-join via numeric ID)
        Hierarchy is now loaded directly from Excel file Column D (MANAGER ID)
        """
        from core.models import Employee
        ...
        try:
            employees = Employee.objects.filter(manager_id__isnull=False)
            
            for emp in employees:
                try:
                    if emp.manager_id:
                        count += 1
                        logger.info(f"  Hierarchy: ID={emp.id} ({emp.rm_name}) → ID={emp.manager_id.id} ({emp.manager_id.rm_name})")
                except Exception as e:
                    logger.warning(f"  Could not verify {emp.rm_name}: {e}")
                    continue
            
            logger.info(f"[OK] Validated {count} employee hierarchy relationships")
            return count
    ...
    def _extract_hierarchy_structure(self, file_path):
        """
        Extract and build hierarchy structure from Excel file (Wirecode_wise_RMdetails.xlsx)
        
        Processes:
        - Reads Excel file with employee hierarchy data
        - Builds full hierarchy paths for each employee
        - Creates columns for different hierarchy levels (Leader, Manager, Employee, MA)
        
        Args:
            file_path: Path to the Excel file containing hierarchy data
            
        Returns:
            DataFrame with hierarchy structure (Leader, Manager, Employee, MA columns)
        """
        import pandas as pd
        ...
        try:
            ...
            # Ensure manager_id is numeric (NaN for top level)
            df["manager_id"] = pd.to_numeric(df["manager_id"], errors="coerce")
            
            # Build dictionary for quick lookup
            employee_dict = df.set_index("employee_id").to_dict("index")
            
            # Recursive function to build hierarchy path
            def get_hierarchy(emp_id):
                path = []
                current_id = emp_id
                
                while pd.notna(current_id):
                    emp = employee_dict.get(current_id)
                    if not emp:
                        break
                    path.insert(0, emp["employee_name"])  # insert at beginning
                    current_id = emp["manager_id"]
                return path
            
            # Build hierarchy paths for each employee
            df["hierarchy_path"] = df["employee_id"].apply(get_hierarchy)
            
            # Determine maximum depth (limit to 4 levels: Leader, Manager, Employee, MA)
            max_depth = 4
            
            # Expand hierarchy into separate columns
            for i in range(max_depth):
                df[f"Level_{i+1}"] = df["hierarchy_path"].apply(
                    lambda x: x[i] if len(x) > i else None
                )
            
            # Create final dataframe with renamed hierarchy columns
            df_final = df[["Level_1", "Level_2", "Level_3", "Level_4"]].copy()
            df_final.columns = ["Leader", "Manager", "Employee", "MA"]
            
            logger.info(f"[OK] Extracted hierarchy structure with {len(df_final)} records")
            return df_final
            
        except Exception as e:
            logger.error(f"Error extracting hierarchy structure from {file_path}: {e}")
            return pd.DataFrame()
