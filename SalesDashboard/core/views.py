...
def dashboard_view(request):
    ...
    def get_all_subordinates(emp):
        """Recursively get all subordinate RM names under an employee (full tree, NOT including emp itself)"""
        names = []
        # Get direct subordinates only (NOT including the manager/emp themselves)
        for subordinate in Employee.objects.filter(manager_id=emp.id):
            if subordinate.rm_name:
                names.append(subordinate.rm_name)
            names.extend(get_all_subordinates(subordinate))
        return names
    
    # Role-based filtering - CRITICAL: enforce at data level using UserProfile.employee link
    if user_role == 'L':  # Leader - can see ALL data
        # Leaders have full access to all records
        pass
    
    elif user_role == 'M':  # Manager - can ONLY see data from their team
        # Use UserProfile.employee link directly (more reliable than name matching)
        if user_profile and user_profile.employee:
            manager_emp = user_profile.employee
            
            # Get all RMs under this manager (including multi-level subordinates)
            subordinate_rms = get_all_subordinates(manager_emp)
            # ENFORCE: Only show data from their team
            records_queryset = records_queryset.filter(rm_name__in=subordinate_rms)
        else:
            # No employee linkage - block all data for security
            records_queryset = records_queryset.none()
    
    else:  # RM/MA - can ONLY see their own data
        # Use UserProfile.employee link directly
        if user_profile and user_profile.employee:
            # Filter to show only this RM's own data
            records_queryset = records_queryset.filter(rm_name=user_profile.employee.rm_name)
        else:
            # No linkage - block all data for security
            records_queryset = records_queryset.none()
    
    # === BUILD DROPDOWNS FIRST (before applying RM/MA filters) ===
    # This ensures dropdowns show all available options for the user's role
    all_managers = []
    all_rms = []
    all_mas = []
    
    # === BUILD DROPDOWNS BASED ON USER ROLE ===
    
    if user_role == 'L':  # Leader - can see ALL RMs and ALL MAs, can select ANY manager
        # Get ONLY L1 Managers - those reporting directly to the Leader
        ...
        # Build RMs from the FILTERED records_queryset
        all_rms = list(records_queryset.values_list('rm_name', flat=True).distinct().exclude(rm_name__isnull=True).exclude(rm_name__exact='').order_by('rm_name'))
        
        # If manager is selected, filter RMs to only show that manager's subordinates
        if selected_manager:
            try:
                manager_emp = Employee.objects.get(rm_name=selected_manager)
                subordinate_rms = get_all_subordinates(manager_emp)
                # Filter to only those who exist in the filtered data
                all_rms = [rm for rm in all_rms if rm in subordinate_rms]
            except Employee.DoesNotExist:
                pass
        
        # Build MA dropdown from filtered RMs
        if selected_rm:
            all_mas = list(records_queryset.filter(
                rm_name=selected_rm
    ...
    elif user_role == 'M':  # Manager - can see ONLY their own team, manager filter frozen
        # Use UserProfile.employee link directly
        if user_profile and user_profile.employee:
            manager_emp = user_profile.employee
            # Get all RMs under this manager (full tree)
            subordinate_rms = get_all_subordinates(manager_emp)
            # Show all RMs under this manager from the filtered data
            all_rms = list(records_queryset.filter(
                rm_name__in=subordinate_rms
            ).values_list('rm_name', flat=True).distinct().order_by('rm_name'))
            # IMPORTANT: Show manager dropdown as frozen to their own name
            all_managers = [manager_emp.rm_name]
            
            # Build MA dropdown for managers
            if selected_rm:
                all_mas = list(records_queryset.filter(
                    rm_name=selected_rm
                ).exclude(ma_name__isnull=True).exclude(ma_name__exact='').values_list('ma_name', flat=True).distinct().order_by('ma_name'))
        ...
        if user_profile and user_profile.employee:
            # Show only this RM's own data
            all_rms = [user_profile.employee.rm_name]
            # Get MAs for this RM
            all_mas = list(records_queryset.filter(
                rm_name=user_profile.employee.rm_name
            ).exclude(ma_name__isnull=True).exclude(ma_name__exact='').values_list('ma_name', flat=True).distinct().order_by('ma_name'))
        else:
            all_rms = []
            all_mas = []
        # RM/MA roles don't see manager dropdown (all_managers stays empty)
    
    # === NOW apply additional filters (AFTER dropdown building) ===
    if selected_manager and user_role == 'L':  # Only Leaders can change manager filter
    ...
    # RM/RM selection (applies to all roles)
    if selected_rm:
        records_queryset = records_queryset.filter(rm_name=selected_rm)
    
    # MA selection (applies to all roles)
    if selected_ma:
