...
print("="*120)
print("LOAD SALES DATA - FINAL VERSION")
print("="*120)
...
for file_path in sorted(brk_dir.glob('*.xlsx')):
    ...
    try:
        ...
        for idx, row in df.iterrows():
            try:
                ...
                total_equity_cash_turnover = safe_decimal(row.get('Total Equity Cash Turnover', 0))
                total_equity_fno_turnover = safe_decimal(row.get('Total Equity FnO Turnover', 0))
                total_equity_turnover = safe_decimal(row.get('Total Equity Turnover', 0))
                total_turnover = safe_decimal(row.get('Total Turnover', 0))
                ...
                SalesRecord.objects.create(
                    ...
                    equity_futures_turnover=futures_turnover,
                    equity_options_turnover=options_turnover,
                    total_equity_cash_turnover=total_equity_cash_turnover,
                    total_equity_fno_turnover=total_equity_fno_turnover,
                    total_equity_turnover=total_equity_turnover,
    ...
    try:
        ...
        for idx, row in df.iterrows():
            try:
                ...
                # Create MF record without employee link (aggregate data)
                SalesRecord.objects.create(
